...
const Slider = ({ data }) => {
  ...
  useEffect(() => {
    const id = setInterval(nextSlide, 5000); // Auto slide every 5 seconds
    setIntervalId(id);
  ...
  return (
    <div className="slider select-none flex justify-center w-full items-center max-md:h-[450px] md:h-[560px] relative">
      <div className="slides relative w-full h-full">
        {(data.length > 0 ? data : slides).map((slide, i) => (
          <div
            key={slide.id || slide._id}
            style={{
              transition: 'opacity 1s ease, z-index 0s',
              opacity: i === currentSlide ? 1 : 0, // Set opacity for the active slide
              zIndex: i === currentSlide ? 10 : 0,  // Set z-index for active slide to be on top
            }}
            className={`slide absolute left-0 top-0 py-8 gap-4 h-full w-full lg:text-white flex max-lg:flex-col justify-center items-center ${
          ...
          >
            <div className="flex flex-col px-6 gap-2 rounded-lg justify-center items-center lg:w-2/5 lg:h-full lg:bg-sky-500 md:w-full max-lg:h-2/5 slantBox">
              <h1 className="md:text-5xl font-semibold relative md:z-50 max-md:text-4xl">
                {slide.name}
              </h1>
              <h2 className="text-base max-md:font-light md:text-slate-300 truncate max-w-40 text-center">
                {slide.content || slide.description}
              </h2>
              <NavLink
                to={`product/${slide._id || slide.id}`}
                className="px-3 py-2 mt-2 bg-sky-500 lg:bg-slate-200 text-slate-900 rounded-md"
            ...
            <div className="flex justify-center items-center md:w-3/5 md:h-full max-md:h-[270px] max-md:w-[220px]">
              <img
                src={slide.img || slide.images[0].secure_url}
                alt="img"
                className="max-md:w-full select-none drag-none rounded-lg md:w-3/5 max-md:h-full md:h-full max-lg:h-3/5"
              />
            </div>
          </div>
        ))}
